...
export default function Clientes() {
  ...
  useEffect(() => {
    if (modalNuevo || modalEditar) {
      sessionStorage.setItem("floppa_wip", "clientes")
    } else {
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 16 }}
          ...
          <div style={{ background: "#1d1b12", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "36px 32px", width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 64px rgba(0,0,0,0.6)" }}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
              ...
              <button onClick={guardarEdicion} disabled={guardando} style={{ flex: 1, padding: "11px", background: "linear-gradient(135deg, var(--accent), var(--accent))", border: "none", borderRadius: 10, color: "white", fontSize: 13, fontWeight: 700, cursor: "pointer", opacity: guardando ? 0.5 : 1 }}>
            ...
            <div className="historial-tabs" style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "0 24px" }}>
              {[
                ...
                <button key={t.key} onClick={() => setTabActiva(t.key)} style={{
                  ...
                  color: tabActiva === t.key ? "var(--accent)" : "#6b7280",
                ...
                <div>
                  ...
                  {ventas.map(v => (
                    <div key={v.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: 14, marginBottom: 8 }}>
                        ...
                        <div style={{ marginTop: 8, background: "rgba(255,255,255,0.03)", borderRadius: 8, padding: "6px 10px", border: "1px solid rgba(255,255,255,0.06)" }}>
                          ...
                          {v.pagos.map((p: any, i: number) => (
                            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, padding: "3px 0", borderBottom: i < v.pagos.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                              ...
                              <button onClick={() => imprimirRecibo(p, v)} style={{ background: "rgba(111,125,73,0.15)", color: "var(--accent)", border: "none", borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontSize: 11, fontWeight: 600, flexShrink: 0, marginLeft: 8 }}>🖨️ Recibo</button>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      <button onClick={() => reimprimirFactura(v)} style={{ marginTop: 10, marginLeft: v.estado === "cobrada" && (!v.pagos || v.pagos.length === 0) ? 6 : 0, background: "rgba(111,125,73,0.15)", color: "var(--accent)", border: "none", borderRadius: 7, padding: "5px 12px", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                        🖨️ Presupuesto
                      </button>
                    </div>
                  ))}
                    ...
                    return (
                      <div key={v.id} style={{ background: "rgba(251,191,36,0.05)", border: "1px solid rgba(251,191,36,0.2)", borderRadius: 12, padding: 16, marginBottom: 12 }}>
                          ...
                          <div style={{ marginTop: 10, background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "8px 12px" }}>
                            ...
                            {v.pagos.map((p: any, i: number) => (
                              <div key={i} className="pago-row" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, padding: "4px 0", borderBottom: i < v.pagos.length - 1 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
                                ...
                                <button onClick={() => imprimirRecibo(p, v)} style={{ background: "rgba(111,125,73,0.15)", color: "var(--accent)", border: "none", borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontSize: 11, fontWeight: 600, flexShrink: 0, marginLeft: 8 }}>Recibo</button>
                              </div>
                            ))}
                          </div>
                        )}
                        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                          ...
                          <button onClick={() => reimprimirFactura(v)} style={{ padding: "9px 14px", background: "rgba(111,125,73,0.15)", border: "none", borderRadius: 8, color: "var(--accent)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>🖨️</button>
                        </div>
                      </div>
                    )
                  })}
                    ...
                    <div key={v.id} style={{ background: "rgba(74,222,128,0.04)", border: "1px solid rgba(74,222,128,0.12)", borderRadius: 10, padding: 12, marginBottom: 8, opacity: 0.85 }}>
                      ...
                      {v.pagos.map((p: any, i: number) => (
                        <div key={i} className="pago-row" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, padding: "3px 0", flexWrap: "wrap", gap: 4 }}>
                          ...
                          <button onClick={() => imprimirRecibo(p, v)} style={{ background: "rgba(111,125,73,0.15)", color: "var(--accent)", border: "none", borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontSize: 11, flexShrink: 0 }}>Recibo</button>
                        </div>
                      ))}
                    </div>
                  ))}
              ...
              {[25, 50, 75, 100].map(pct => {
                ...
                return (
                  <button key={pct} onClick={() => setMontoPago(String(val))} style={{
                    ...
                    background: Number(montoPago) === val ? "var(--accent)" : "rgba(255,255,255,0.05)",
